 ...
 * Navigation/media remote buttons are sent through the pointer-input socket.
 */
export class LgWebOsAdapter implements TvAdapter {
  ...
  private async connectRegistered(device: TvDevice, pairingType: "PROMPT" | "PIN" = "PROMPT"): Promise<WebSocket> {
      ...
      payload: {
        ...
        manifest: {
          ...
          permissions: [
            ...
            "CONTROL_AUDIO", "CONTROL_INPUT_MEDIA_PLAYBACK", "CONTROL_INPUT_TV",
            "CONTROL_POWER", "CONTROL_TV_SCREEN", "READ_APP_STATUS",
            "READ_CURRENT_CHANNEL", "READ_INPUT_DEVICE_LIST", "READ_NETWORK_STATE",
            "READ_RUNNING_APPS", "READ_TV_CHANNEL_LIST", "READ_POWER_STATE",
    ...
    if (first?.type === "registered") {
      ...
      if (key) {
        TokenVault.saveToken(device.id, key);
      }
      return ws;
    }

    // For PROMPT/PIN the TV can send a response followed by a registered message.
    if (first?.type === "response" && first.payload?.pairingType) {
      return ws;
    }
  ...
  private waitForOpen(ws: WebSocket, timeoutMs: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => {
        cleanup();
        reject(new Error("LG webOS WebSocket connection timed out."));
      }, timeoutMs);
      const cleanup = () => {
  ...
  private nextMessage(ws: WebSocket, timeoutMs: number): Promise<any> {
    return new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => {
        cleanup();
        reject(new Error("Timed out waiting for LG webOS response."));
      }, timeoutMs);
      const cleanup = () => {
        window.clearTimeout(timer);
      ...
      const onError = () => {
        cleanup();
        reject(new Error("LG webOS WebSocket error."));
      };
  ...
  private async request(device: TvDevice, uri: string, payload: Record<string, any> = {}): Promise<any> {
    ...
    try {
      ...
      if (response?.type === "error") {
        throw new Error(response.error || response.payload?.errorText || `LG request failed: ${uri}`);
      }
      return response;
    } finally {
      try { ws.close(); } catch {}
  ...
  private async runCommand(device: TvDevice, command: RemoteCommandType, value?: any): Promise<void> {
    const pointerMap: Partial<Record<RemoteCommandType, string>> = {
      HOME: "HOME", BACK: "BACK", MENU: "MENU", UP: "UP", DOWN: "DOWN", LEFT: "LEFT", RIGHT: "RIGHT",
      OK: "ENTER", VOLUME_UP: "VOLUMEUP", VOLUME_DOWN: "VOLUMEDOWN", MUTE: "MUTE",
      CHANNEL_UP: "CHANNELUP", CHANNEL_DOWN: "CHANNELDOWN", PLAY: "PLAY", PAUSE: "PAUSE", STOP: "STOP",
      PREVIOUS: "REWIND", NEXT: "FASTFORWARD", REWIND: "REWIND", FAST_FORWARD: "FASTFORWARD",
      EXIT: "EXIT", INFO: "INFO", GUIDE: "GUIDE", DASH: "DASH",
      COLOR_RED: "RED", COLOR_GREEN: "GREEN", COLOR_YELLOW: "YELLOW", COLOR_BLUE: "BLUE",
    ...
    if (pointer) {
      await this.pointerCommand(device, "button", pointer);
      return;
    }
    ...
    switch (command) {
      case "POWER":
        ...
        return;
      case "SET_VOLUME":
        await this.request(device, "ssap://audio/setVolume", { volume: Math.max(0, Math.min(100, Number(value))) });
        return;
      case "PLAY_PAUSE":
        await this.pointerCommand(device, "button", "PLAY");
        return;
      case "SET_CHANNEL":
        await this.request(device, "ssap://tv/openChannel", { channelId: String(value) });
        return;
      case "PREV_CHANNEL":
        await this.pointerCommand(device, "button", "CHANNELDOWN");
        return;
      case "INPUT":
        await this.request(device, "ssap://tv/getExternalInputList");
        return;
      case "SET_INPUT":
        await this.request(device, "ssap://tv/switchInput", { inputId: String(value) });
        return;
      case "TEXT_INPUT":
        await this.request(device, "ssap://com.webos.service.ime/insertText", { text: String(value ?? ""), replace: 0 });
        return;
      case "KEYBOARD_ENTER":
        await this.request(device, "ssap://com.webos.service.ime/sendEnterKey");
        return;
      case "KEYBOARD_BACKSPACE":
        await this.request(device, "ssap://com.webos.service.ime/deleteCharacters", { count: 1 });
        return;
      case "LAUNCH_APP":
        await this.request(device, "ssap://system.launcher/launch", { id: String(value) });
        return;
      case "TOUCHPAD_MOVE":
        await this.pointerCommand(device, "move", value || {});
        return;
      case "TOUCHPAD_CLICK":
        await this.pointerCommand(device, "click");
        return;
      default:
        throw new Error(`LG webOS command not implemented: ${command}`);
    }
  }
  ...
  async executeCommand(device: TvDevice, command: RemoteCommandType, value?: any): Promise<CommandExecutionResult> {
    ...
    try {
      ...
      return {
        ...
        latencyMs: Math.round(performance.now() - startTime),
        protocol: "lg_webos_ssap_direct",
        error: err?.message || `Could not reach LG webOS TV at ${device.ip}:${device.port || 3000}.`
      };
    }
  }
  ...
  async authenticate(device: TvDevice, pin: string, _clientName = "Universal Remote"): Promise<{ success: boolean; token?: string; error?: string }> {
    try {
      const hasToken = !!(TokenVault.getToken(device.id) || device.token);
      const pairingType = hasToken ? "PROMPT" : (pin ? "PIN" : "PROMPT");
      ...
      if (pairingType === "PIN") {
        ...
        if (pinResponse?.type === "error") {
          ws.close();
          return { success: false, error: pinResponse.error || pinResponse.payload?.errorText || "LG webOS pairing rejected." };
        }
      }

      const registered = await this.nextMessage(ws, 10000);
      const token = registered?.payload?.["client-key"];
      if (registered?.type !== "registered" || !token) {
        ws.close();
        return { success: false, error: "LG TV did not complete pairing." };
      }
  ...
  async ping(device: TvDevice): Promise<{ online: boolean; latencyMs?: number; error?: string }> {
    ...
    try {
      ...
      return { online: false, latencyMs: Math.round(performance.now() - start), error: err?.message || "LG webOS TV unreachable." };
    }
  }
}
